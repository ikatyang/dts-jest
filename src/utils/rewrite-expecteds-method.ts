...
export const rewrite_expecteds_method = (target_expecteds: Expected[]): void => {
  const groups: Group[] = [];
  const grouped_expecteds = target_expecteds.reduce<{[index: number]: Expected[]}>(
    (current_grouped_expecteds, expected) => {
      ...
      function safe_push(index: number) {
        current_grouped_expecteds[index] = default_to(current_grouped_expecteds[index], [] as Expected[]);
  ...
  const only_group_index = find_index(groups, group => group.method === GroupMethod.Only);
  ...
  function rewrite_expecteds(expecteds: Expected[]) {
    const only_test_index = find_index(expecteds, expected => expected.method === TestMethod.Only);
    if (only_test_index !== -1) {
      expecteds[only_test_index].method = TestMethod.Test;
      skipify_expecteds(expecteds.filter((_, index) => index !== only_test_index));
    }
  }
