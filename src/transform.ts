...
export const transform = (source_text: string, source_filename: string, _jest_config?: any) => {
  ...
  return triggers.reduce<string[]>(
    (transformed_line_texts, trigger, index) => {
      ...
      write_group_close_if_preset(trigger.group !== undefined && index === triggers.length - 1);

      last_group = trigger.group;
      return transformed_line_texts;

      function write(str: string, trailing_semicolon: boolean) {
        transformed_line_texts[trigger.line] += str + (trailing_semicolon ? ';' : '');
      }
      function write_group_open_if_preset(value: boolean) {
        if (value) { write(create_group_expression(trigger.group!, {type: 'open'}), false); }
      }
      function write_group_close_if_preset(value: boolean) {
        if (value) { write(create_group_expression(trigger.group!, {type: 'close'}), true); }
      }
    },
